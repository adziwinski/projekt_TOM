function [ ] = compare22( A,G,x, errorSize )
    ...
    subplot(1,2,1)
     ...
     ylabel('y')
     grid minor;
     legend(sprintf('równanie y = %.1f*x + %.1f', A(1,1)/A(1,2), G(1)/A(1,2)), sprintf('równanie y = %.1f*x + %.1f', A(2,1)/A(2,2), G(2)/A(2,2)),...
    ...
    subplot(1,2,2)
     ...
     text(-0.2,1.2, 'Porównanie metod (2 równania i 2 niewiadome, sta³a lambda)', 'HorizontalAlignment', 'center',...
    ...
    subplot(1,2,1)
     ...
     ylabel('y')
     grid minor;
     legend(sprintf('równanie y = %.1f*x + %.1f', A(1,1)/A(1,2), G(1)/A(1,2)), sprintf('równanie y = %.1f*x + %.1f', A(2,1)/A(2,2), G(2)/A(2,2)), 'Additive ART','Multiplicative ART', 'Pseudoodwrotnoœæ', 'Lewe dzielenie', 'Location' , 'southwest')
    ...
    subplot(1,2,2)
     ...
     text(-0.2,1.2, 'Porównanie metod (2 równania i 2 niewiadome, lambda zmniejszana w ka¿dej iteracji)', 'HorizontalAlignment', 'center',...
        'FontSize', 16 , 'Units', 'normalized')
end
